  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <QuickStat label="Total Revenue" value={formatPrice(stats.total)} color="text-[#c9a96e]" />
      ...
      <div className="bg-white border border-[#e5e7eb] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="p-12 text-center">
            <ShoppingCart size={40} className="text-[#c9a96e] mx-auto mb-3" />
                ...
                {filtered.map((order) => (
                  <tr key={order.id} className="hover:bg-[#fafaf9] transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/orders/${order.id}`} className="text-[#c9a96e] font-semibold hover:underline text-sm">#{order.orderNumber}</Link>
                    </td>
                    <td className="px-4 py-3">
