  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <QuickStat label="VIP (100K+)"       value={stats.vip.toString()}      color="text-[#E10600]" />
        ...
        {[
          ...
          <button key={f.val} onClick={() => setFilter(f.val as typeof filter)}
            className={cn("px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px",
              filter === f.val ? "border-[#E10600] text-[#E10600]" : "border-transparent text-[#6b7280] hover:text-[#1a1a1a]")}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white border border-[#e5e7eb] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="p-12 text-center">
            <Users size={40} className="text-[#E10600] mx-auto mb-3" />
                  ...
                  <tr key={c.id} className="hover:bg-[#fafaf9] transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-[#f5f0e8] flex items-center justify-center flex-shrink-0">
                          <span className="text-xs font-bold text-[#E10600]">{getInitials(c.name)}</span>
                        </div>
                        <div>
                          <Link href={`/customers/${c.id}`} className="text-sm font-semibold text-[#1a1a1a] hover:text-[#E10600]">
                            {c.name}
                          </Link>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        <a href={`mailto:${c.email}`} className="flex items-center gap-1.5 text-xs text-[#6b7280] hover:text-[#E10600]"><Mail size={11} />{c.email}</a>
                        {c.phone && <a href={`tel:${c.phone}`} className="flex items-center gap-1.5 text-xs text-[#6b7280] hover:text-[#E10600]"><Phone size={11} />{c.phone}</a>}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-[#E10600]">{formatPrice(c.totalSpent)}</td>
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/customers/${c.id}`} className="text-[#6b7280] hover:text-[#E10600]"><ChevronRight size={16} /></Link>
                    </td>
                  </tr>
                ))}
              </tbody>
