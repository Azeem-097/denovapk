    ...
    return (
      <div className="border border-[#e5e7eb] p-4 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-xs font-semibold uppercase tracking-wider text-[#c9a96e]">{title}</p>
          <button type="button" onClick={() => addLink(settingKey)}
            className="text-[10px] font-semibold text-[#c9a96e] hover:text-[#b8955a] underline">
            + Add Link
          </button>
        </div>
        {links.length === 0 ? (
            ...
            {links.map((link, i) => (
              <div key={i} className="flex items-center gap-2 bg-[#fafaf9] p-2 border border-[#e5e7eb]">
                <input type="text" value={link.label} placeholder="Label"
                  ...
                  className="flex-1 px-2 py-1.5 text-xs border border-[#e5e7eb] focus:border-[#c9a96e] focus:outline-none" />
                <input type="text" value={link.href} placeholder="/page-url"
                  onChange={(e) => updateLink(settingKey, i, "href", e.target.value)}
                  className="flex-1 px-2 py-1.5 text-xs border border-[#e5e7eb] focus:border-[#c9a96e] focus:outline-none font-mono" />
    ...
    <div>
      ...
      <div className="space-y-6">
        ...
        <div className="border border-[#e5e7eb] p-4 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-[#c9a96e]">Brand Section</p>
