...
// Helpers (duplicated here to keep script standalone)
...
async function seed() {
  ...
  const tables = [
    ...
    "wishlists", "reviews", "product_variants", "product_images",
    "products", "collections", "discounts", "addresses",
    "sessions", "users", "admins",
    "newsletter", "contact_messages",
    "shipping_methods", "shipping_zones", "settings"
  ];
  ...
  await db.execute({
    sql: `INSERT INTO admins (id, name, email, password, role, isActive, createdAt, updatedAt)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [adminId, "Hassan Sheikh", "admin@denovapk.com", adminPassword, "SUPER_ADMIN", 1, now(), now()],
  });
  ...
  await db.execute({
    sql: `INSERT INTO users (id, name, email, password, phone, isActive, createdAt, updatedAt)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [userId, "Ayesha Malik", "ayesha@example.com", userPassword, "+92 300 1234567", 1, now(), now()],
  });
  const addrId = genId();
  await db.execute({
  ...
  for (const c of collections) {
    await db.execute({
      sql: `INSERT INTO collections (id, name, slug, description, image, isActive, createdAt, updatedAt)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [c.id, c.name, c.slug, c.desc, c.img, 1, now(), now()],
    });
    console.log(`  ✅ ${c.name}`);
  }
  console.log("");
  ...
  for (const p of products) {
    ...
    await db.execute({
      sql: `INSERT INTO product_images (id, productId, url, alt, isPrimary, sortOrder)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [genId(), pid, p.img, p.name, 1, 0],
    });
