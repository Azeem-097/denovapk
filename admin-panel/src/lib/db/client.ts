...
function validateTursoConfig() {
  ...
  if (fixed.length > 0) {
    for (const msg of fixed) {
      console.warn(YELLOW + "[Turso Admin] AUTO-FIXED: " + RESET + msg);
    }
    console.warn(YELLOW + "[Turso Admin] Consider fixing your .env file manually — remove the stray quote character." + RESET);
  }

  // Report blocking errors
  if (errors.length > 0) {
    console.error("\n" + RED + BOLD + "═══════════════════════════════════════════════════════════════" + RESET);
    console.error(RED + BOLD + "  ❌  TURSO DATABASE CONFIGURATION ERROR (ADMIN PANEL)" + RESET);
  ...
  if (process.env.NODE_ENV !== "production") {
    console.log(GREEN + "[Turso Admin] ✓ Database configuration loaded" + RESET);
  }
}
