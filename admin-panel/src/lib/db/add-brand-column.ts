/**
 * Adds `brand` column to products table.
 * Safe to re-run — checks if column exists first.
 *
 * Usage:  npx tsx src/lib/db/add-brand-column.ts
 */
...
async function run() {
  ...
  const hasColumn = info.rows.some((r) => (r.name as string) === "brand");

  if (hasColumn) {
    console.log("Column `brand` already exists on products. Nothing to do.");
    process.exit(0);
  }
