...
export function DateRangeSelector({ current }: { current: string }) {
  ...
  return (
    <div className="bg-white border border-[#e5e7eb] p-3 flex items-center gap-2 flex-wrap">
      ...
      {pending && (
        <span className="ml-2 inline-flex items-center gap-1 text-[10px] text-[#c9a96e]">
          <span className="w-3 h-3 border-2 border-[#c9a96e] border-t-transparent rounded-full animate-spin" />
          Updating...
        </span>
      )}
    </div>
