...
const LENS_WIDTH      = 120;   // px lens over source (square)
const LENS_HEIGHT     = 120;   // px lens over source (square)
const PANEL_MARGIN    = 24;   // horizontal margin between gallery column and panel
const PANEL_BOTTOM    = 0;    // px above viewport bottom edge (0 = flush to bottom)
...
//  Aligned to the top of the hovered image, extends down to
...
function ZoomPanel({ state }: { state: ZoomState }) {
  ...
  const panelLeftRaw  = hostRect.right + PANEL_MARGIN;
  const panelRightRaw = window.innerWidth - PANEL_MARGIN;
  let   panelWidth    = panelRightRaw - panelLeftRaw;
  let   actualLeft    = panelLeftRaw;

  if (panelWidth < 320) {
    // Fallback: force usable width, pin to right
    panelWidth = Math.max(400, Math.round(window.innerWidth * 0.45));
    actualLeft = window.innerWidth - panelWidth - PANEL_MARGIN;
  }

  // ─── Vertical placement ─────────────────────────────────
  // Top edge of the panel aligns with the top edge of the hovered image.
  // Height extends from there down to (viewport bottom - PANEL_BOTTOM).
  let panelTop    = Math.max(0, sourceRect.top);
  let panelHeight = window.innerHeight - panelTop - PANEL_BOTTOM;

  // Safety: enforce a minimum panel height
  if (panelHeight < 300) {
    panelHeight = 300;
    panelTop    = window.innerHeight - panelHeight - PANEL_BOTTOM;
