          ...
          <div>
            ...
            {filteredOrders.length === 0 ? (
              <div className="bg-white border border-[#e5e7eb] p-10 text-center">
                <Package size={40} className="text-[#c9a96e] mx-auto mb-4" />
                  ...
                  <FadeIn key={order.id} delay={i * 60}>
                    ...
                    >
                      ...
                      <div className="px-5 py-3 border-b border-[#e5e7eb] bg-[#fafaf9] flex items-center justify-between gap-3 flex-wrap">
                        ...
                        <span className={`text-[10px] px-2.5 py-1 rounded-full font-semibold uppercase tracking-wide ${STATUS_COLORS[order.status]}`}>
                          {order.status}
                        </span>
                      </div>
                      ...
                      <div className="px-5 py-4 flex items-center gap-4 flex-wrap">
                        <div className="flex -space-x-3 flex-shrink-0">
                          {order.items.slice(0, 3).map((item) => (
                            <div
                              key={item.id}
                              className="relative w-14 h-14 bg-[#fafaf9] border-2 border-white overflow-hidden"
                            >
                              <Image
                                src={item.image}
                                alt={item.name}
