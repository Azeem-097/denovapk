...
export function ReviewStep({ onBack }: Props) {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!shippingData) {
      showToast({ type: "error", message: "Shipping info missing" });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        showToast({ type: "error", message: data.error || "Order failed" });
        setPlacing(false);
        return;
      }
  ...
  return (
    <div className="px-4 py-3.5 flex items-start justify-between gap-3">
      ...
      <button type="button" onClick={onEdit}
        className="text-xs text-[#c9a96e] hover:text-[#b8955a] underline flex-shrink-0 mt-0.5">
        Edit
      </button>
    </div>
  );
