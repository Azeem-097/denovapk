          ...
          <div className="flex items-center gap-3 mb-6">
            <form onSubmit={handleSubmit} className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 text-base text-[#1a1a1a] border border-[#e5e7eb] focus:border-[#F97316] focus:outline-none placeholder:text-[#6b7280]/60"
              />
              {query && (
                <button
                  type="button"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {recentSearches.map((s) => (
                      <button
                        ...
                        className="text-sm text-[#1a1a1a] bg-[#fafaf9] border border-[#e5e7eb] hover:border-[#F97316] px-3 py-1.5 transition-colors"
                      >
                        {s}
                      </button>
                    ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {TRENDING.map((s) => (
                    <button
                      key={s}
                      onClick={() => handleQuickSearch(s)}
                      className="text-sm text-white bg-[#1a1a1a] hover:bg-[#F97316] px-3 py-1.5 transition-colors"
                    >
                      {s}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {popular.map((p) => (
                    ...
                    >
                      ...
                      <p className="text-xs font-medium text-[#1a1a1a] group-hover:text-[#F97316] transition-colors line-clamp-1">
                        {p.name}
                      </p>
  ...
  return (
    <div>
        ...
        {results.map((p) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-wider text-[#F97316]">{p.collection}</p>
              <p className="text-sm font-medium text-[#1a1a1a] group-hover:text-[#F97316] transition-colors line-clamp-1">
                {p.name}
              </p>
              <p className="text-sm font-bold text-[#1a1a1a] mt-0.5">{formatPrice(p.price)}</p>
            </div>
            <ArrowRight size={14} className="text-[#6b7280] group-hover:text-[#F97316] flex-shrink-0" />
          </Link>
        ))}
      </div>
      ...
      <Link
        ...
        onClick={onSelect}
        className="mt-6 block text-center bg-[#1a1a1a] text-white py-3 text-sm font-semibold tracking-wide hover:bg-[#F97316] transition-colors"
      >
        View all results
      </Link>
    </div>
